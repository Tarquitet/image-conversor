...
class UniversalOmegaConverter:
    ...
    def build_ui(self):
        ...
        ttk.Label(header, text="OMEGA CONVERTER V6 (Smart Resize)", font=("Helvetica", 18, "bold"), bootstyle="primary").pack(side="left")
        ...
        sp_q.bind("<FocusOut>", self.on_ui_change) # FIX 1
        ...
        # COMBOBOX DE REDIMENSIÓN CON LOGICA INTELIGENTE
        ...
        cb_rm.bind("<<ComboboxSelected>>", self.on_resize_mode_change) # FIX 2: Nueva función lógica
        
        # SPINBOX DE TAMAÑO CON RANGO ALTO Y EVENTOS
        sp_s = ttk.Spinbox(hf3, from_=1, to=20000, textvariable=self.var_resize_val, width=6); 
        sp_s.pack(side="right")
        sp_s.bind("<Return>", self.on_ui_change)
        sp_s.bind("<FocusOut>", self.on_ui_change) # FIX 3: Actualizar al salir del campo
    ...
    # --- LÓGICA INTELIGENTE DE UI ---
    def on_resize_mode_change(self, event=None):
        """Cambia el valor por defecto inteligentemente al cambiar de modo"""
        mode = self.var_resize_mode.get()
        
        # Si cambiamos a porcentaje, volver a 100 (estándar)
        if mode == "Scale %":
            self.var_resize_val.set(100)
        
        # Si cambiamos a PX, intentar poner el tamaño REAL de la imagen actual
        # para que el usuario no vea "100px" (que es minúsculo)
        elif self.current_item and self.original_image:
            w, h = self.original_image.size
            if mode == "Width Px":
                self.var_resize_val.set(w)
            elif mode == "Height Px":
                self.var_resize_val.set(h)
            elif mode == "Longest Px":
                self.var_resize_val.set(max(w, h))
        else:
            # Si no hay imagen cargada, poner un estándar HD
            self.var_resize_val.set(1920)
            
        self.on_ui_change()

    # --- LÓGICA DE LISTA Y BORRADO ---
    def on_list_motion(self, event):
        idx = self.listbox.nearest(event.y)
        if idx == -1: 
            self.btn_del_item.place_forget(); return
    ...
    def process_image(self, pil_img, item):
        ...
        if pil_fmt=="AVIF": args.update({"quality":q, "speed":6})
            ...
            # NUEVO: Si la calidad es menor a 100, reducimos drásticamente el peso
            # reduciendo la cantidad de colores permitidos (Cuantización).
            if q < 100:
                # Mapea la calidad (1-99) a cantidad de colores (2 a 256)
                num_colors = max(2, int((q / 100) * 256))
                
                # Para evitar perder transparencia limpia, aseguramos modo RGBA
                if target.mode != "RGBA" and target.mode != "P": 
                    target = target.convert("RGBA")
                    
                target = target.quantize(colors=num_colors, method=Image.MAXCOVERAGE)
