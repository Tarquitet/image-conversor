...
class UniversalOmegaConverter:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet - OMEGA Converter V4")
        ...
        # Variable para controlar el botón de borrar
    ...
    def build_ui(self):
        ...
        ttk.Label(header, text="OMEGA CONVERTER V4", font=("Helvetica", 18, "bold"), bootstyle="primary").pack(side="left")
        ...
        # Inicialmente oculto, se mostrará al mover el mouse
        self.btn_del_item = ttk.Button(self.listbox, text="×", width=2, bootstyle="danger-outline", command=self.delete_hovered_item)
        
        # Eventos para efecto "Hover"
        self.listbox.bind("<Motion>", self.on_list_motion)
        self.listbox.bind("<Leave>", lambda e: self.btn_del_item.place_forget())
    ...
    # --- LÓGICA DE LISTA Y HOVER ---
    def on_list_motion(self, event):
        # 1. Detectar índice bajo el mouse
        idx = self.listbox.nearest(event.y)
        if idx == -1: return
        
        # 2. Verificar que el mouse esté realmente sobre el item (no fuera por abajo)
        ...
        # 3. Mostrar botón "X" a la derecha del item
        self.hovered_idx = idx
        
        # Calcular posición: Ancho total lista - 30px
        btn_x = self.listbox.winfo_width() - 30 
        btn_y = bbox[1] + (bbox[3]//2) - 12 # Centrado vertical
        
        self.btn_del_item.place(x=btn_x, y=btn_y, width=25, height=25)
    ...
    def delete_hovered_item(self):
        ...
        if 0 <= self.hovered_idx < len(self.items):
            # Si borramos el que se está viendo, limpiar vista
